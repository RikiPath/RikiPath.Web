...
// Reuse converter by importing logic inline (same as import-missing-desktop)
...
for (const item of RETRY) {
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      console.log(`Retry ${item.name} attempt ${attempt}`);
      await sleep(1500 * attempt);
      const screen = await getScreen(item.id);
      if (!screen?.htmlCode?.downloadUrl) throw new Error('no htmlCode');
      ...
      if (attempt === 3) console.log('GIVE UP', item.name);
    }
  }
}
